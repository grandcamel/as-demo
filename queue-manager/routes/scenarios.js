...
function register(app) {
  app.get('/api/scenarios/:name', (req, res) => {
    ...
    // Using path.relative is more robust across platforms (Windows/Unix)
    ...
    fs.readFile(resolvedPath, 'utf8', (err, markdown) => {
      ...
      const html = scenarioTemplate
        ...
        .replace(/\{\{PLATFORM\}\}/g, escapeHtml(scenario.platform || 'unknown'))
  ...
  // List all scenarios grouped by platform
  app.get('/api/scenarios', (req, res) => {
    const grouped = config.getScenariosByPlatform();
    res.json(grouped);
  });
}
