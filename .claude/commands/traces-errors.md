...
```bash
ssh root@143.110.131.254 'curl -s "http://localhost:3200/api/search" \
...
```bash
ssh root@143.110.131.254 'curl -s "http://localhost:3200/api/traces/<TRACE_ID>" | jq'
```

Or view traces in Grafana (requires active session):

```
https://demo.assistant-skills.dev/grafana/explore?orgId=1&left={"datasource":"tempo","queries":[{"refId":"A","queryType":"traceql","query":"{status=error}"}]}
```
